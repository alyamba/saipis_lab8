...
const formatNumbersInArray = (array, digitsAfterDot) => array?.map(arr => arr.map(el => +el.toFixed(+digitsAfterDot)))
  ...
  fs.exists(DATA_DB_PATH2, (isExists) => {
    if (!isExists) {
      const csvWriter = createCsvWriter({
        path: DATA_DB_PATH2,
        header: [
          { id: "array", title: "arrayDescending" },
        ],
      });
